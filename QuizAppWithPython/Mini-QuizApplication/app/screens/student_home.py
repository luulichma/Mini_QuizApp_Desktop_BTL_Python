...
class StudentHomeScreen(Screen):
    ...
    def load_my_classes(self):
        """Tải và hiển thị danh sách các lớp học mà sinh viên đã tham gia."""
        ...
        try:
            ...
            for cls in classes:
                item = Factory.MyClassListItem()
                item.ids.my_class_name.text = cls['class_name']
                item.ids.my_class_description.text = cls.get('description', 'Không có mô tả')
                my_classes_list_layout.add_widget(item)
